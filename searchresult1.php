...
<html lang="en">
	...
	<body>
		<?php
			...
			$query = "SELECT * FROM answer WHERE questuon_id IN(select question_id from question where question LIKE '%$search_query%')";
		?>
					...
					<table class="ui celled unstackable table">
						...
						if ($result = $conn->query($query)) {
							...
							while ($row = $result->fetch_assoc()){
								printf("<tr>");
								printf("<td>%s</td> <td>%s</td> ",  $row["answer_id"], $row["answer"]);
								printf("</tr>");
								$ser++;
							}
						}
